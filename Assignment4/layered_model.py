...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    #outputs = layers.Dense(output_length, activation=activation_f)(inputs)
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    #inputs = tf.keras.Input(shape=input_length)
    #x = layers.Dense(hidden_layer_size, activation=activation_func_array[0])(inputs)
    #outputs = layers.Dense(output_length, activation=activation_func_array[1])(x)
    #return Model(inputs, outputs)
    model = keras.Sequential([
        layers.Input(shape=(input_length,)),
